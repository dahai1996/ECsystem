...
/**
 *基于内容的相似推荐
 * 源数据：商品数据 Product，内有字段表示商品的特征，如 富勒|鼠标|电子产品|好用|外观漂亮
 * 功能：
 * 1.基于内容的商品离线推荐，得出表 ContentBasedProductRecs ，表数据为 ：rdd【ProductRecs】.toDF()
 * 思路： 与Lixian_Recommender过程相似，不同的是 一个采用ALS，一个采用TF-IDF,两种不同的方法来得到特征向量
 * 1.从原始数据表Rating中得到数据
 * 2.用TF-IDF提取商品特征向量
 * 3.为了得出所有商品两两互相的相似度，
 *  传入参数为所有商品向量与自身的笛卡尔积，并去除自身向乘
 * 4.保留相似度>0.4的，并排序之后，存入mongodb
 */
