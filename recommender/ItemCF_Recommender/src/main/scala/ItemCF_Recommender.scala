...
/*基于item-CF的离线推荐
源数据：Rating表，格式：ProductRating( userId: Int, productId: Int, score: Double, timestamp: Int )
功能：
1.根据用户对商品的评论，产生每个商品的相似商品列表 ItemCFProductRecs
思路：
1.计算商品的同现相似度： （同时评论两个商品的用户）/（评论a商品的人数）（评论b商品的人数）开根号
2.读取数据，去掉不需要的数据最后格式为：toDF("userId", "productId", "score")
3.对数据进行处理，找到同时评论一个商品的两个用户id，（后续需要去掉自身*自身），并计数，得到公式的分子
4.找到a商品的评论数，b商品的评论数，得到公司的分母
  上述两步较为复杂，利用jion和sql来处理
5.保存到MongoDB的表ItemCFProductRecs中
* */
object ItemCF_Recommender {
  // 定义常量和表名
